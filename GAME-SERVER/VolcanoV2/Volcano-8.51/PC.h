...
static bool (*CantBuild)(SDK::UWorld*, SDK::UObject*, FVector, FRotator, char, void*, char*) = decltype(CantBuild)(GetOffsetBRUH(0x1330D70)); // 0x1330D70
...
void ServerSetTeamHook(AFortPlayerControllerAthena* PC, uint8 NewTeam)
{
    ...
    if (auto PlayerState = Cast<AFortPlayerStateAthena>(PC->PlayerState))
    {
        ...
        {
            auto CurrentUniqueId = decltype(GetGameState()->GameMemberInfoArray.Members[i].MemberUniqueId){};
            CurrentUniqueId = GetGameState()->GameMemberInfoArray.Members[i].MemberUniqueId;

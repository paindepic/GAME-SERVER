...
namespace BotSystem
{
    ...
    public:
        ...
        void DetermineNewState(FBotPlayer& Bot)
        {
            ...
            float EnemyDistance = NearestEnemy ? FVector::Distance(Bot.Pawn->K2_GetActorLocation(), 
        ...
        AActor* FindNearestEnemy(FBotPlayer& Bot)
        {
            if (!Bot.Pawn)
                return nullptr;
            ...
            for (int i = 0; i < GameState->PlayerArray.Num(); i++)
            {
                ...
                // Skip bots and teammates
                if (PlayerState->TeamIndex == Bot.Controller->PlayerState->TeamIndex)
                    continue;
        ...
        void ExecuteFleeBehavior(FBotPlayer& Bot)
        {
            ...
            FVector ToEnemy = Bot.TargetEnemy->K2_GetActorLocation() - Bot.Pawn->K2_GetActorLocation();
            ToEnemy.Normalize();
            
            FVector FleeDirection = -ToEnemy * 2000.0f; // Move 2000 units away
        ...
        int GetBotCount() const { return Bots.size(); }
